...
contract BalanceChecker {
  ...
  */
  ...
  function tokenAllowance(address user, address token, address spender) public view returns (uint) {
    // check if token is actually a contract
    uint256 tokenCode;
    assembly { tokenCode := extcodesize(token) } // contract code size
  
    // is it a contract and does it implement allowance 
    if (tokenCode > 0 && token.call(bytes4(0xdd62ed3e), user, spender)) {  
      return Token(token).allowance(user, spender);
    } else {
      return 0;
    }
  }
  
  function allowances(address[] users, address[] tokens, address spender) external view returns (uint[]) {
    uint[] memory addrAllowances = new uint[](tokens.length * users.length);
    
    for(uint i = 0; i < users.length; i++) {
      for (uint j = 0; j < tokens.length; j++) {
        uint addrIdx = j + tokens.length * i;
        if (tokens[j] != address(0x0)) { 
          addrAllowances[addrIdx] = tokenAllowance(users[i], tokens[j], spender);
        } else {
          addrAllowances[addrIdx] = users[i].balance; // ETH balance    
        }
      }  
    }
